...
We have now produced an attribute matrix consisting of features that are correctly formatted for use in machine learning models.
...
\Function{OneVsRestClassify}{$X, C$}
...
\label{4.5}
Hyperparamter tuning is a crucial step to finding the best model. As data our data is complex and highly dimensional, we will need to apply a hyperparameter tuning to find the best parameters of our models (SVC and RFC). We will use a Random Search algorithm to find the best hyperparameters (see Algorithm 5). A Grid Search would've been another alternative, which would exhaustively search for the best model in a given hyperparameter space.
